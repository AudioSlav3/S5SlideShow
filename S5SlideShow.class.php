...
class S5SlideShow
{
     ...
     */
    public function __construct(Title $sTitle, ?string $sContent = null, ?array $attr = null)
    {
        if (!($sTitle instanceof Title)) {
        ...
        if ($sContent === null) {
            $this->pageContent = $sTitle->getContent();
        } else {
     ...
     */
    function setAttributes(array $attr): void // Strict type hinting for arguments and return value
    {
        ...
        // Default values with array merge
        ...
        // Boolean value with type casting
        $attr['scaled'] = (bool) ($attr['scaled'] === 'true' || $attr['scaled'] === 'yes' || $attr['scaled'] === 1);
        ...
        $this->attr = array_merge($this->attr, $attr); // Merge with existing attributes
    }
     ...
     */
    function check_slide_heading(DOMNode $node): ?array
    {
     ...
     */
    function transform_section_slides(string $content): string // Specify return type
    {
        ...
        $this->heading_re = null;  // Initialize to avoid potential errors
        $this->inc_re = null;
        $this->center_re = null;
    
        if ($this->attr['headingmark']) {
            $this->heading_re = '/'.preg_quote(str_replace('/', '\\/', $this->attr['headingmark'])).'/';
        }
        if ($this->attr['incmark']) {
            $this->inc_re = '/'.preg_quote(str_replace('/', '\\/', $this->attr['incmark'])).'/';
        }
        if ($this->attr['centermark']) {
            $this->center_re = '/'.preg_quote(str_replace('/', '\\/', $this->attr['centermark'])).'/';
        }
     ...
     */
    function loadContent(?string $content = null): array
    {
     ...
     */
    function parse(string $text, bool $inline = false, ?string $title = null): string
    {
        global $wgOut;
        ...
        $text = trim(str_replace("__TOC__", '', $text));
    ...
    function getHeadItems(): string
    {
        ...
        $wgOut->getContext()->setSkin(new SkinApiClean());
        $s = $wgOut->headElement(new SkinApiClean());
    
        $pattern = '/<script[^<>]*>.*?<\/script>|'.
            '<link[^<>]*rel="stylesheet"[^<>]*>|'.
            '<meta[^<>]*name="ResourceLoaderDynamicStyles"[^<>]*>/is';
    
        preg_match_all($pattern, $s, $m, PREG_PATTERN_ORDER);
     ...
     */
    function genSlideFile(bool $printPageSize = false): void
    {
        ...
        if ($printPageSize) {
            // Default DPI
            $dpi = 96;
            $replace['[styleurl]'] .= '&print=' . implode('x', array_map('intval', $printPageSize));
     ...
     */
    public static function styleReplaceUrl(string $skin, string $m): string
    {
        ...
        // FIXME: Remove hardcoded extensions/S5SlideShow/
     ...
     */
    public static function genStyle(string $skin, bool $print = false): void
    {
        global $wgOut;
        ...
        foreach (S5SlideShowHooks::$styles as $k => $file) {
            ...
            $c = preg_replace_callback('#url\(([^\)]*)\)#is', function ($m) use ($skin) {
                return S5SlideShow::styleReplaceUrl($skin, $m[1]);
            }, $c);
     ...
     */
    public static function slideshow_legacy(string $content, array $attr, object $parser, ?object $frame = null): string
    {
     ...
     */
    public static function clone_options_parse(string $content, Parser $parser, bool $inline = false): string
    {
        if (!$parser->mTitle) {
     ...
     */
    public static function slideshow_view(string $content, array $attr, Parser $parser, ?Frame $frame = null, string $addmsg = ''): string
    {
        ...
        // FIXME: remove hardcoded values (.png, /extensions/S5SlideShow/, "Slide Show")
    ...
    function slideshow_parse(string $content, array $attr, object $parser): string
    {
        $attr['content'] = $content;
        $this->setAttributes($attr);
        return '';
    ...
    public static function slides_view(string $content, array $attr, object $parser): string
    {
        if (!empty($attr['split'])) {
    ...
    function slides_parse(string $content, array $attr, $parser): void
    {
        if (isset($attr['split'])) {
            $slides = preg_split('/' . preg_quote($attr['split'], '/') . '/', $content);
        } else {
            $slides = [$content];
        }
        ...
        foreach ($slides as $c) {
            $this->slides[] = [
                'content' => trim($c),
                ...$attr, // Spread operator for attributes
                'title' => '',
                'incremental' => false,
                'center' => false,
            ];
    ...
    public static function slidecss_view(string $content, array $attr, object $parser): string
    {
    ...
    public function slidecss_parse(string $content, array $attr, object $parser): void
    {
        $this->css[] = $content;
    }

    // stub for tag hooks
    public static function empty_tag_hook(): string
    {
        return '';
    }
     ...
     */
    public static function strCheck(string &$haystack, string $needle): bool
    {
        $needle = mb_strtolower($needle);
        $haystackLength = mb_strlen($haystack);
        $needleLength = mb_strlen($needle);
    
        if (mb_strtolower(mb_substr($haystack, 0, $needleLength)) === $needle) {
            $haystack = trim(mb_substr($haystack, $needleLength));
            return true;
        } elseif (mb_strtolower(mb_substr($haystack, $haystackLength - $needleLength, $needleLength)) === $needle) {
            $haystack = trim(mb_substr($haystack, 0, $haystackLength - $needleLength));
            return true;
        }
...
{
    public string $s5skin;
    public string $s5file;
    
    // Create the object and remember s5skin and s5file
    public function __construct(string $title, string $s5skin, string $s5file)
    {
    ...
    public function showMissingArticle(): void
    {
        global $wgOut, $wgRequest, $wgParser;
    
        // Copy-paste from includes/Article.php:
        // Show delete and move logs
        LogEventsList::showLogExtract($wgOut, ['delete', 'move'], $this->mTitle->getPrefixedText(), '', [
            'lim' => 10,
        ...
        $oldid = $this->getOldID();
        if ($oldid) {
            $text = wfMessage('missing-article', $this->mTitle->getPrefixedText(), wfMessage('missingarticle-rev', $oldid)->plain())->plain();
        } else {
...
{
    public function setupSkinUserCss(OutputPage $out): void
    {
