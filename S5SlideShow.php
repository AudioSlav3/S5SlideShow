...
$egS5SlideIncMark = '(step)'; // Use double quotes for string literal in PHP 8.1

// Slides with this text in heading will be shown centered
$egS5SlideCenterMark = '(center)'; // Use double quotes for string literal in PHP 8.1

// Filesystem path to slideshow template file
$egS5SlideTemplateFile = $dir . '/slide.htm'; // Use string concatenation
...
class S5SlideShowHooks
{
    ...
    public static bool $parsingSlide = false;
    ...
    static public function MagicWordwgVariableIDs(&$mVariablesIDs)
    {
    ...
    static public function ParserGetVariableValueSwitch($parser, $varCache, $index, &$ret)
    {
    ...
    static function ImageBeforeProduceHTML(Skin $skin, string $title, File $file, array $frameParams, array $handlerParams, int $time, ?string &$res): bool
    {
        global $wgVersion;
        if (empty(self::$parsingSlide) || !$file || !$file->exists() || !array_key_exists('width', $handlerParams)) {
            return true;
        }
        ...
        $thumb = $file->getUnscaledThumb(isset($hp['page']) ? ['page' => $hp['page']] : null);
        ...
        if (version_compare($wgVersion, '1.22', '>=')) {
            ...
            $thumb->height = ceil($thumb->getHeight() * $hp['width'] / $thumb->getWidth());
        ...
        if (isset($fp['thumbnail'])) {
            ...
            $res = "<div class=\"thumb t{$fp['align']}\" style='border:0'>
                ...
                <div class='thumbcaption'>{$fp['caption']}</div>
            </div>";
        }
    ...
    public static function UnknownAction($action, $article): bool
    {
        ...
        $s5skin = preg_match('/[^\w-]/', $s5skin) ? '' : $s5skin;
    
        $print = $wgRequest->getVal('print');
    
        if ($print) {
            preg_match_all('/\d+/', $print, $print, PREG_PATTERN_ORDER);
            $print = $print[0];
        } else {
            $print = false;
        }
    ...
    static function ArticleFromTitle($title, $article)  // Remove reference
    {
    ...
    {
        return $editpage->mArticle instanceof S5SkinArticle && !$editpage->mArticle->exists()
            ? $editpage->mPreloadText = $editpage->mArticle->getContent() // Use short return syntax
            : true;
    }
}
